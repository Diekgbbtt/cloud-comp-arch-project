...
def edit_job_configuration(yaml_objects: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
	...
	# Job-specific configuration with optimizations for better performance and resource usage
	if job_name == "parsec-streamcluster":
		template_spec["nodeSelector"] = {"cca-project-nodetype": "node-a-8core"}
		requests.update({"cpu": "6", "memory": "8Gi"})  # Increased memory to handle larger working set
		limits.update({"cpu": "7", "memory": "8Gi"})
		container["command"] = ["/bin/sh"]
		container["args"] = ["-c", "taskset -c 1-7 ./run -a run -S parsec -p streamcluster -i native -n 7"]
	# Increased threads to 7 to fully utilize available cores on node-a-8core

	elif job_name == "parsec-freqmine":
		template_spec["nodeSelector"] = {"cca-project-nodetype": "node-b-4core"}
		requests.update({"cpu": "3", "memory": "4Gi"})  # Adjusted memory to fit node-b-4core
		limits.update({"cpu": "4", "memory": "4Gi"})
		container["command"] = ["/bin/sh"]
		container["args"] = ["-c", "./run -a run -S parsec -p freqmine -i native -n 4"]
	# Increased threads to 4 to match node-b-4core's vCPUs

	elif job_name == "parsec-canneal":
		template_spec["nodeSelector"] = {"cca-project-nodetype": "node-a-8core"}
		requests.update({"cpu": "4", "memory": "6Gi"})  # Adjusted memory for better performance
		limits.update({"cpu": "6", "memory": "6Gi"})
		container["command"] = ["/bin/sh"]
		container["args"] = ["-c", "taskset -c 4-7 ./run -a run -S parsec -p canneal -i native -n 4"]
	# Limited to 4 threads to avoid memory controller saturation on node-a-8core

	elif job_name == "parsec-blackscholes":
		template_spec["nodeSelector"] = {"cca-project-nodetype": "node-a-8core"}
		requests.update({"cpu": "2", "memory": "2Gi"})  # Reduced memory to free up for other jobs
		limits.update({"cpu": "3", "memory": "3Gi"})
		container["command"] = ["/bin/sh"]
		container["args"] = ["-c", "taskset -c 2-3 ./run -a run -S parsec -p blackscholes -i native -n 2"]
	# Reduced threads to 2 to avoid interference with memcached

	elif job_name == "parsec-radix":
		template_spec["nodeSelector"] = {"cca-project-nodetype": "node-a-8core"}
		requests.update({"cpu": "1", "memory": "1Gi"})  # Minimal resources to avoid interference
		limits.update({"cpu": "2", "memory": "2Gi"})
		container["command"] = ["/bin/sh"]
		container["args"] = ["-c", "taskset -c 0 ./run -a run -S splash2x -p radix -i native -n 1"]
	# Explicitly pinned to core 0 to avoid interference with memcached

	elif job_name == "parsec-barnes":
		template_spec["nodeSelector"] = {"cca-project-nodetype": "node-b-4core"}
		requests.update({"cpu": "3", "memory": "4Gi"})  # Adjusted to fit node-b-4core
		limits.update({"cpu": "4", "memory": "4Gi"})
		container["command"] = ["/bin/sh"]
		container["args"] = ["-c", "./run -a run -S splash2x -p barnes -i native -n 4"]
	# Increased threads to 4 to utilize node-b-4core's resources
