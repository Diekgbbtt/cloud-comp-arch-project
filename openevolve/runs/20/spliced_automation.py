...
def edit_job_configuration(yaml_objects: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    - containers[0]["args"]: list of launch cmd chars, template: ["-c", "taskset -c <vCPUs_range> ./run -a run -S parsec -p <job> -i native -n <software_threads_count>"]
    - CPU pinning through process affinity with tasksets("-c", "taskset -c <vCPUs_range>), in which vCPUs_range is a 0-indexed list with syntax: <lower-boundary>-<upper-boundary>, e.g. 2-3 or 0-4 or just collapses to the vCPU number if bound to just one core, e.g. 1
    ...
    # Predefined configurations for each job type
    job_configurations = {
        "parsec-blackscholes": {
            ...
            "args": [
                "-c",
                "taskset -c 1-3 ./run -a run -S parsec -p blackscholes -i native -n 2",
            ],
        },
        "parsec-canneal": {
        ...
        "parsec-streamcluster": {
            ...
            "args": [
                "-c",
                "taskset -c 4-7 ./run -a run -S parsec -p streamcluster -i native -n 2",
            ],
        },
        "parsec-freqmine": {
            "nodeSelector": {"cca-project-nodetype": "node-b-4core"},
            ...
            "args": [
                "-c",
                "taskset -c 0-3 ./run -a run -S parsec -p freqmine -i native -n 2",
            ],
        },
        "parsec-radix": {
            "nodeSelector": {"cca-project-nodetype": "node-a-8core"},
            ...
            "args": [
                "-c",
                "taskset -c 4 ./run -a run -S splash2x -p radix -i native -n 1",
            ],
        },
        "parsec-barnes": {
            "nodeSelector": {"cca-project-nodetype": "node-b-4core"},
            ...
            "args": [
                "-c",
                "taskset -c 0 ./run -a run -S splash2x -p barnes -i native -n 1",
            ],
        },
        "parsec-vips": {
            "nodeSelector": {"cca-project-nodetype": "node-b-4core"},
            ...
            "args": [
                "-c",
                "taskset -c 1 ./run -a run -S parsec -p vips -i native -n 1",
            ],
        },
    }
